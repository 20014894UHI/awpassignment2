...
const trimFactor=.25; // Would need to make sure this is < 1
...
// Can also use coordinates of eyes form HAAR_EYE to determine left right eye e.g. as in the example
...
// *** Load glasses image. The parameter -1 reads also alpha channel (if exists) // Or use cv.IMREAD_UNCHANGED);
...
//     .convertTo(alpha, CV_32FC3, 1.0/255); // .cvtColor(cv.COLOR_BGR2HLS);
...
//console.log("glassesBgr.channels: ", glassesBgr.channels); 
//console.log("glassesBgr.type: ", glassesBgr.type);  

// Alt A - other tests on loading the PNG with alpha channel
//  matGlassesAlpha = cv.imread("./glassesalpha.png", -1);
// Create the mask for the glasses
// let glassesMaskTh0031 = matGlassesAlpha.threshold(230, 255, cv.THRESH_BINARY_INV);
// let glassesMaskTh003 =  matGlassesAlpha.threshold(0, 0, 3);

// Alt 2 - 1  load the sunglasses image jpg (no alpha channel available)
// const matGlasses = cv.imread("./sunglasses1.jpg");
// const matGlasses = cv.imread("./sunglasses1.jpg", -1); // will load alpha if present
...
// const glassesRatio_A2 = matGlasses.cols / matGlasses.rows; // Alternative 2 - 
// const imgGlassesGray  = matGlasses.bgrToGray();

// Find a position that we can base the start position for the glasses at
function nudgeGlasses (gStart, fWidth, faceL, glWidth) {
  if (gStart >= faceL + (fWidth*.15))   {
    for (let i = gStart; i = (faceL + fWidth*.15); i--) {
      if (i == faceL + fWidth*.15){
        return i; 
      }
    }
  }
...
function calcTilt(angle) {
  ...
  if (angle < 180) tilt = 180 - angle;
    // anti clockwise
  else if (angle >= 180) tilt = 180 - angle; // clockwise
    return tilt;
  //tilt = angle < 180 ? -(180-angle) :  (180-angle);
}
...
function checkOverlap(ptLeft1, ptRight1, ptLeft2, ptRight2)
{
    ...
    if (ptLeft1.x > ptRight2.x || ptLeft2.x > ptRight1.x)
    return false;
    // If one rectangle is above other
    if (ptRight1.y > ptLeft2.y || ptRight2.y > ptLeft1.y)
        return false;
    return true;
}
...
function withinFrame(x,y, w,h, fw, fh) {
  if (!x || !y ) {return false; } // if undefined 
  if (x > fw || x < 0) {
    return false; }
  if (x + w > fw || x + w < 0)   
  {return false;
...
// Used this prior to using copyTo to place the glasses on the regionGlasses_A2
// Answer by Gavin https://stackoverflow.com/a/57716197/19318647 in 
...
module.exports = function(socket) {
  ...
  setInterval(function() {
    ...
    if (faceResult.objects.length > 0) {
      // Different options/alternatives - use this line or the following line
      ...
      // DEL const faceRectR = new cv.Point(faceRect.x + faceWidth, faceRect.y); 
      ...
      // Example data:
      // faceRegion Mat {step: 1920,elemSize: 3,sizes: [ 190, 190 ],empty: 0,depth: 0,
      // dims: 2,channels: 3,type: 16,cols: 190,rows: 190}
      ...
      const rightEyeResult = rightEyeClassifier.detectMultiScale(grayFaceRegion); // locate right eye
      const leftEyeResult = leftEyeClassifier.detectMultiScale(grayFaceRegion); // locate left eye
      ...
      const glassesProportionateColumns = Math.round(glassesProportionateRows * glassesRatio);
      const mustacheCols =  Math.round(mustacheProportionateRows * mustacheRatio);
      // *** Resize the mask to size of region where the glasses will be placed
      const resizedMustacheMask = mustacheMask.resize(Math.round(mustacheProportionateRows / mustacheRatio), Math.round(mustacheProportionateRows));      
      ...
      const resizedMustacheBGR = mustacheBgr.resize(mustacheProportionateRows,mustacheCols);
      cv.imwrite("./E_ResizedMustacheBlueGreenRed.jpg", resizedMustacheBGR);

      // Alternative 2. GLASSES - before dealing with Alpha channel
      // const resizedGlasses_A2 = matGlasses.resize(glassesProportionateRows,glassesCols_A2);
      // const resizedGlasses_A2 = matGlassessrc.resize(glassesProportionateRows, glassesWidth, interpolation=cv.INTER_AREA);

      // Not using this classifier HAAR_EYE
      // if (eyeResult.objects.length > 0) {
          // const eyeRect = eyeResult.objects[sortByNumDetections(eyeResult)[0]];
          // const eyeRect = sortByNumDetections(eyeResult).slice(0, 2).map((idx) => eyeResult.objects[idx]);
          //if (eyeResult.objects.length > 0) {
          // function angle(originX, originY, targetX, targetY) {
          //eyeRect.forEach((eyeRect) => { drawGreenRect(faceRegion, eyeRect);});
          // }
      // };

      let ptMustacheTL, anyEye, eyePairRect, eyePairEndMiddleR, eyePairEndMiddleL, mouthRect, leftEyeRect, rightEyeRect, noseRect;
      let nose, mouthDetected, leftEye, eyePair, rightEye;  
      let eyeOverlapsMouth, noseAboveMouth, lEyeOverlapsMouth, eyePairOverlapsMouth, isTilted, rEyeOverlapsMouth = false; 
      let leftEyeRelativeToRight = false;
      let ptNoseTopL, noseWidth, eyePairWidth, ptNoseMiddle, ptMouthTopL, ptMouthTopR, mouthWidth,  ptMouthTopMiddle, ptEyePairTopL, ptEyePairBottomL, 
      ptEyePairTopR, ptRightEyeBottomL, ptLeftEyeTopL, ptLeftEyeTopR, ptLeftEyeBottomL, lEyeWidth, rEyeWidth, lEyeMiddle, ptRightEyeTopR, ptRightEyeTopL,glassesOffset;
      let eyeRotation= 0; 
      let glassesWidth=0; 
      var eyes_and_glasses = [];
      var face_and_mustache = [];
      let glassesTopLx, glassesTopLy; // glassesHeight; 
      ...
      if (eyePairResult.objects.length > 0) {
        ...
        ptEyePairBottomL = new cv.Point(eyePairRect.x + eyePairWidth, eyePairRect.y - eyePairRect.height);
        eyePairEndMiddleR = new cv.Point(eyePairRect.x + eyePairWidth, eyePairRect.y - (eyePair.height/2));
        eyePairEndMiddleL = new cv.Point(eyePairRect.x, eyePairRect.x - (eyePair.height/2));
      ...
      // cv.imwrite("./frameWithGlassesOut.jpg",
      // overlayOnto(resizedGlassesGray, frame, faceRect.x + rightEyeRect.x, faceRect.y + rightEyeRect.y );
      // cv.imwrite("./frameWithMustacheOut.png",
      // overlayOnto(resizedMustache, frame, (faceRect.x+ rightEyeRect.x *1.2 ), faceRect.y + rightEyeRect.y *2    ));
      ...
      if (mouthResult.objects.length > 0) {
        ...
        ptMouthTopMiddle = new cv.Point(ptMouthTopL.x + (mouthWidth/2), ptMouthTopR.y);
        ptMouthMiddle = calcRectCentre(mouthRect);
        
        // If mouth overlaps left or right eye or eye pair not reliable 
        if (leftEye)  { lEyeOverlapsMouth = overlapVertically(ptMouthTopL, ptLeftEyeBottomL); }
        if (rightEye) { rEyeOverlapsMouth  = overlapVertically(ptMouthTopL, ptRightEyeBottomL); } 
        if (eyePair)  { eyePairOverlapsMouth = overlapVertically(ptMouthTopL, ptEyePairBottomL);}
      ...
      const diagonalGlassesSquare = (glassesProportionateColumns*glassesProportionateColumns) + 
      (glassesProportionateRows*glassesProportionateRows);
      ...
      //const color = new cv.Vec(1, 0, 0);
          
      /*******************************************************/
      /* Evaluate scenarios depending on what is detected   */ 
      if (showGlasses == true) {
        //glassesWidth = Math.round(faceWidth * .7); // fallback where no better alternative  
        if (!faceRect) {console.log("Face not detected this frame"); }
        if (faceRect && !anyEye) {console.log("Eyes not detected in face this frame");}
        ...
        // ************************
          ...
          leftEyeRelativeToRight = rectsInSequenceLeftRight(leftEyeRect, rightEyeRect);
          ...
          if (leftEyeRelativeToRight && !eyesOverlap) { //     if (lEyeMiddle.x > rEyeMiddle.x) {
          ...
          // Or not in sequence but eyePair available 
          else if ((leftEyeRelativeToRight && eyesOverlap && eyePair) || 
            ...
            if (ptEyePairTopR.x > ptLeftEyeTopR.x && ptEyePairTopR.x > ptRightEyeTopR.x) {        
              console.log("          1.2.1 eye pair further over than both LE and RE detected");
          ...
          // Also no useful rotation information available 
                   ...
                   (!leftEyeRelativeToRight && !eyePair)) {
           console.log("1.3. (b) LE/RE ARE LE in sequence, BUT overlap AND no eye pair ")
           console.log("     (a) LE and RE not in correct relative position, no eye pair ")
            ...
            glassesWidth = Math.round(faceWidth * .7); 
            ...
            if (nose) {
              if (ptRightEyeTopR.x < ptLeftEyeTopR.x && ptRightEyeTopL.x > ptLeftEyeTopL.x
                  & ptNoseTopL.x < ptLeftEyeTopL.x) { // position from 'left' eye'
            ...
            glassesTopLx = Math.round((faceWidth*.1));
            glassesWidth = faceWidth * .7; 
            ...
            // else {
            //   glassesOffset = Math.round(rEyeWidth/2);
            console.log("   glasses out of range?", glassesTopLx + glassesWidth > frame.cols);
        ...
        glassesTopLy = Math.round(ptEyePairTopL.y  + paddingGlassesT - (eyePairRect.height)); 
        ...
        // ****************
        else if (rightEye && (!eyePair || !leftEye)) {         //    else if (leftEye && nose && (!eyePair || !rightEye)) {
          // Nose not a good indicator  in this scenario as can be over to the side 
        ...
        if (anyEye && !glassesWidth) {"Error: glasses width is undefined"}
        else if (anyEye && glassesWidth && glassesWidth < 0) {"Error: glasses width is undefined or < 0"}
        else if (anyEye && glassesWidth == 0) {"Error: glasses width cannot be 0"}     
        else if (anyEye && glassesWidth && glassesWidth !== 0) {
          resizedGlassesMask = glassesMask.resize(Math.round(glassesWidth / glassesRatio), Math.round(glassesWidth));
        ...
        if (anyEye && glassesWidth > 0) {
          ...
          const glassesWithinFrame = withinFrame(glassesTopLx, glassesTopLy, glassesProportionateColumns,glassesProportionateRows, frame.cols, frame.rows);
          if (glassesTopLy > 0 && glassesTopLx > 0 && glassesWithinFrame) { 
            ...
            const paddedResizedGlassesBGRchannels = 
            resizedGlassesBGR.copyMakeBorder(paddingGlassesT, paddingGlassesB, paddingGlassesL, 
              paddingGlassesR, cv.BORDER_CONSTANT, transColor);
            ...
            const regionGlassesPadded = frame.getRegion(
              new cv.Rect(Math.round(glassesTopLx),Math.round(glassesTopLy),paddedGlassesWidth,paddedGlassesHeight));
            ...
            const regionGlassesPaddedCropped = frame.getRegion(new cv.Rect(Math.round(glassesTopLx),
              Math.round(glassesTopLy+(paddedGlassesHeight*trimFactor)),
              paddedGlassesWidth,paddedGlassesHeight*(1-trimFactor*2)));
            ...
            const rotMatGlassesPaddedTM = cv.getRotationMatrix2D(
              new cv.Point(Math.round(paddedGlassesHeight / 2), 
              Math.round(paddedGlassesWidth / 2)),  
              eyeRotation );
            ...
            const rotatedGlassesPadded = paddedResizedGlassesBGRchannels
              .warpAffine(rotMatGlassesPaddedTM);
            ...
            const rotatedGlassesPaddedCropped = rotatedGlassesPadded
              .getRegion(new cv.Rect(0, 0+paddedGlassesHeight*trimFactor, 
                        paddedGlassesWidth, paddedGlassesHeight*(1-trimFactor*2)));
            ...
            const rectRegionGlassesPadded = new cv.Rect(
              Math.round(glassesTopLx), Math.round(glassesTopLy), 
              paddedGlassesWidth, paddedGlassesHeight);
            ...
            const rectRegionGlassesPaddedCropped = new cv.Rect(
            ...
            if (eyeRotation == 0) {isTilted = false }
            else isTilted =true; 
            ...
            // cv.imwrite("./ainvMask.jpg", invMask);
            // const output = mask.hMul(rotatedGlassesPaddedCropped).add(glassesMask_inv.hMul(regionGlassesBgPaddedCropped));
            // cv.imwrite('./aoutput.jpg', output);
            // //  cv.imwrite('./afg.jpg', fg);
            // //cv.imwrite('./aallones.jpg',allOnes);
            //  cv.imwrite('./ainvMask.jpg', invMask);
            //  face_and_mustache = regionMustacheBgPaddedCropped.add(rotatedMustachePadded);
            //@
            // Add background and foreground -> image of the eyes with transparent glasses overlaid
            // const eyes_and_glasses = regionGlassesBg.add(regionGlassesBGR_FG);
         
            // Below does not work as data type needs to be a Rect
            // eyes_and_glasses.copyTo(frame.getRegion(regionGlasses));
            // Following format from Alt 1 resizedGlasses_A2.copyTo(frame.getRegion(regionGlasses_A2));
            // useful ternary tp select the method
            // mRgba = img.channels === 1 ? img.cvtColor(cv.COLOR_GRAY2RGBA) : img.cvtColor(cv.COLOR_BGR2RGBA);
            // Copy the eyes and glasses to the rect area of the color roi
            //  new cv.Rect(Math.round(faceRect.x + rightEyeRect.x*.8),  Math.round(faceRect.y + rightEyeRect.y), glassesProportionateColumns, glassesProportionateRows)
            // eyes_and_glasses.copyTo(frame.getRegion(rectRegionGlasses));
            ...
            // The Alt 2 - copied the blue glasses with the white surrounding background
            //  resizedGlasses_A2.copyTo(frame.getRegion(regionGlasses_A2));
          } 
          else if (glassesTopLy < 0 && glassesTopLx < 0 ||  !glassesWithinFrame) {
         //   !withinFrame(glassesTopLx, glassesTopLy, glassesProportionateColumns,glassesProportionateRows, frame.cols, frame.rows)) {
            console.log("   Cannot position glasses outside of frame");
          }
        }    // End common block for all scenarios 

        //else    {//  console.log("6. Unhandled scenario");}
      ...
      if (showMustache==true){
        ...
        if (mouthDetected && !eyeOverlapsMouth) { 
          ptMustacheTL = new cv.Point(Math.round(ptMouthTopMiddle.x -mustacheCols/2), 
          Math.round(ptMouthTopL.y - mouthRect.height * 2)); //(mustacheProportionateRows * 2));
          }
          // Fallback use nose if mouth position not reliable or mouth not detected
        else if (!mouthDetected && nose){
          ptMustacheTL = new cv.Point(ptNoseMiddle.x - mustacheCols/2, noseRect.y - mustacheProportionateRows/3);
        }
        if (mouthDetected || nose) {//} && !eyeOverlapsMouth) {
          ...
          const paddedResizedMustacheBGRchannels = resizedMustacheBGR.copyMakeBorder(
            paddingMustacheT, paddingMustacheB, paddingMustacheL, paddingMustacheR, 
            cv.BORDER_CONSTANT, transColor);
          ...
          const moustacheWithinFrame = 
            withinFrame(moustacheTopLx, moustacheTopLy, mustacheCols, 
              paddedMoustacheHeightTrimmed, frame.cols, frame.rows);
        
          if (moustacheTopLy > 0 && moustacheTopLx > 0 && moustacheWithinFrame) {
            // ROIS
            // The old one, now not valid
            // const regionMustache = frame.getRegion(new cv.Rect(Math.round(moustacheTopLx), Math.round(moustacheTopLy), mustacheCols, mustacheProportionateRows));
            ...
            const regionMustachePaddedCropped = frame.getRegion(new cv.Rect(Math.round(moustacheTopLx),
            Math.round(moustacheTopLy+(paddedMustacheHeight*trimFactor)),
            paddedMustacheWidth,paddedMoustacheHeightTrimmed));
            ...
            const regionMustacheBgPaddedCropped = regionMustachePaddedCropped.bitwiseAnd(regionMustachePaddedCropped, mustacheMask_inv);
            ...
            const rotatedMustachePaddedCropped = rotatedMustachePadded.getRegion(new cv.Rect(0, 0+paddedMustacheHeight*trimFactor, paddedMustacheWidth, paddedMoustacheHeightTrimmed));
            ...
            const rectRegionMustachePadded = new cv.Rect(Math.round(moustacheTopLx), Math.round(moustacheTopLy), paddedMustacheWidth, paddedMustacheHeight);
            ...
            const rectRegionMustachePaddedCropped = new cv.Rect(Math.round(moustacheTopLx), Math.round(moustacheTopLy+paddedMustacheHeight*trimFactor), paddedMustacheWidth, paddedMoustacheHeightTrimmed);
          ...
          //For cropped one - but crashing/needs tweaking 
          //else if (moustacheTopLy < 0 && moustacheTopLx < 0 || !withinFrame(moustacheTopLx, moustacheTopLy, mustacheCols,paddedMoustacheHeightTrimmed, frame.cols, frame.rows)) {
          else if (moustacheTopLy < 0 && moustacheTopLx < 0 ||
            !moustacheWithinFrame
  //  !withinFrame(moustacheTopLx, moustacheTopLy, mustacheCols, paddedMoustacheHeightTrimmed, frame.cols, frame.rows)
            ) {
        ...
        // **************
        // END Moustache (new) approach that adds padding to stop cropping of rotated image 
        // **************     
   
        // **************
        // END Moustache previous approaches - rotated image gets cropped and also the previous one that does not 
        // deal with the alpha channel so background is copied 
        // **************     
   
        //   if (withinFrame(m1TopLx, m1TopLy, mustacheCols, mustacheProportionateRows, frame.cols, frame.height)) {
        //     const regionMustache = new cv.Rect(Math.round(m1TopLx),Math.round(m1TopLy), mustacheCols,mustacheProportionateRows );
        //     const matRegionMustache = frame.getRegion(regionMustache);
        //     const regionMustacheBg = matRegionMustache.bitwiseAnd(matRegionMustache,tmustacheMask_inv);
        //     const regionMustacheBgrFg = resizedMustacheBgr.bitwiseAnd(resizedMustacheBgr,resizedMustacheMask);
        //     const rectRegionTmustache = new cv.Rect(Math.round(m1TopLx),Math.round(m1TopLy),mustacheCols,mustacheProportionateRows);
        //     const rotMatMustache = cv.getRotationMatrix2D(new cv.Point(mustacheCols / 2, mustacheProportionateRows / 2),eyeRotation);
        //     const rotatedMustache = resizedMustacheBgr.warpAffine(rotMatMustache);

        //     // old way without dealing with alpha channel 
        //     //resizedMustacheBgr.copyTo(frame.getRegion(rectRegionTmustache));
        //     if (leftEye && rightEye && isTilted){ //} && (!tiltToleranceExceeded)) {
        //         face_and_mustache = regionMustacheBg.add(rotatedMustache);
        //         face_and_mustache.copyTo(frame.getRegion(regionMustache)); }
        //     else if ((leftEye && rightEye) && !isTilted){
        //       face_and_mustache = regionMustacheBg.add(regionMustacheBgrFg);
        //       face_and_mustache.copyTo(frame.getRegion(regionMustache)); 
        //     }
        //   }
        //   else if (!withinFrame) {console.log("Region outside the frame");}
        //   // ************ End moustache ***********/
        // }
